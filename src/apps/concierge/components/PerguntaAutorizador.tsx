        ...
        {resultados.length >= 7 || algumNomeLongo ? (
          <div className="flex flex-col gap-2.5 mt-3">
            {resultados.map((nome) => (
              ...
              >
                <UserCheck size={18} strokeWidth={2.25} className="text-[var(--es-navy)] shrink-0" />
                <span className="text-[17px] font-medium text-[var(--es-ink)] truncate min-w-0 flex-1">{nome}</span>
              </button>
            ))}
          </div>
        ) : (
          <div className="flex flex-wrap gap-2.5 mt-3">
            {resultados.map((nome) => (
              <SugestaoChip key={nome} label={nome} icon={<UserCheck size={15} strokeWidth={2.25} />} onClick={() => preencher(nome)} />
            ))}
          </div>
        )}
        {resultados.length === 0 && (
          <p className="text-[17px] text-[var(--es-ink-3)] mt-3">Nenhum autorizador encontrado.</p>
        )}
      </div>
